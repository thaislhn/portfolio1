@font-face {
  font-family: 'PP Mondwest';
  src: url('PPMondwest-Regular.woff2') format('woff2'),
      url('PPMondwest-Regular.woff') format('woff');
  font-weight: normal;
  font-style: normal;
  font-display: swap;
}

* {
  margin: 0;
  padding: 0;
  box-sizing: border-box;
}

body {
  font-family: 'PP Mondwest', monospace;
  font-size: 20px;
  background-color: #e6e6e6;
  height: 100vh;
  display: flex;
  justify-content: center;
  align-items: center;
  color: #000000;
}

h1 {
  font-family: 'PP Mondwest'; 
  color: #000000;
}

.folder-grid{
  color: #000000;
}

.folder span {
  font-family: 'PP Mondwest';
  font-size: 20px !important;
  letter-spacing: 1px;
}

.desktop {
  width: 100%;
  height: 100%;
  padding: 20px;
  position: relative;
}

#background-video {
  position: fixed;
  top: 0;
  left: 0;
  width: 100vw;
  height: 100vh;
  object-fit: cover;
  z-index: -1;
}

.project-detail {
  position: fixed;
  top: 0;
  left: 0;
  width: 100%;
  height: 100%;
  background-color: rgba(0, 0, 0, 0.7); 
  z-index: 1000;
  display: none;
  overflow-y: auto;
}

.window {
  background: #ffffff;
  width: 90%;
  width: 550px;
  overflow: hidden;
  border: 2px solid #000000;
  box-shadow: 3px 2px 0 rgba(0, 0, 0, 0.5);    
  position: absolute;
  top: 15%;
  left: 50%;
  transform: translate(-50%, -50%);
  transition: all 0.3s ease;
}

.window-content {
  padding: 18px;
  background: #ffffff;
}

.window-header {
  background: #000000;
  color: #d9ef35;
  padding: 8px 15px;
  display: flex;
  justify-content: space-between;
  align-items: center;
  cursor: move;
}

.window-title {
  font-size: 28px
}

.close-button {
  background: none;
  border: none;
  color: #fff;
  font-size: 20px;
  cursor: pointer;
  padding: 0 5px;
}

.folder-grid {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(120px, 1fr));
  width: 500px;
  height: 130px;
  }

  .folder {
  display: flex;
  flex-direction: column;
  align-items: center;
  cursor: pointer;
  padding: 10px;
  transition: all 0.2s ease;
  }

  .folder:hover {
  background-color: rgba(91, 88, 91, 0.1);
  transform: translateY(-2px);
  }

  .folder:active {
  transform: translateY(0);
  }
  
  .folder-icon {
  width: 164px;
  height: 64px;
  margin-bottom: 8px;
  transition: transform 0.2s ease;
  }
  .folder:hover .folder-icon {
  transform: scale(1.1);
  }

  .folder span {
  text-align: center;
  font-size: 14px;
  }
  
  /* About Windows Stack */
  .about-window-stack {
  position: absolute;
  bottom: 20px;
  left: 5%;
  width: 350px;
  height: 350px;
  z-index: 10;
  transition: all 0.3s ease;
  }

  .about-window {
  width: 100%;
  position: absolute;
  border: 2px solid #000000;
  box-shadow: 2px 2px 0 rgba(0, 0, 0, 0.5);
  background: white;
  }

  .about-window:nth-child(1) {
  top: 0;
  left: 0;
  z-index: 3;
  }

  .about-window:nth-child(2) {
  top: 10px;
  left: 10px;
  z-index: 2;
  }

  .about-window:nth-child(3) {
  top: 20px;
  left: 20px;
  z-index: 1;
  }

  .project-window-stack {
  position: absolute;
  top: 250px;
  left: 50%;
  transform: translateX(-50%);
  width: 500px;
  height: 700px;
  z-index: 20;
  }

  .projects-window {
  width: 100%;
  height: 100%;
  position: absolute;
  border: 2px solid #000000;
  box-shadow: 2px 2px 0 rgba(0, 0, 0, 0.5);
  background: white;
  box-sizing: border-box;
  overflow: hidden;
  }
  .projects-window:nth-child(1) {
  top: 0;
  left: 0;
  z-index: 4;
  }
  .projects-window:nth-child(2) {
  top: 10px;
  left: 10px;
  z-index: 2;
  }
  .projects-window:nth-child(3) {
  top: 20px;
  left: 20px;
  z-index: 1;
  }

  .projects-window .window-content {
  height: calc(100% - 30px);
  overflow: auto;
  box-sizing: border-box;
  }

  .cv-window-stack {
  position: absolute;
  top: 80px;
  left: 75%;
  width: 320px;
  height: 320px;
  z-index: 15;
  transform: rotate(0deg);
  transition: all 0.3s ease;
  }

  .cv-window {
  width: 100%;
  height: 100%;
  position: absolute;
  border: 2px solid #000000;
  box-shadow: 2px 2px 0 rgba(0, 0, 0, 0.5);
  background: white;
  transform: rotate(0deg);
  }

  .cv-window:nth-child(1) {
  top: 0;
  left: 0;
  z-index: 3;
  }

  .cv-window:nth-child(2) {
  top: 10px;
  left: 10px;
  z-index: 2;
  }
  .cv-window:nth-child(3) {
  top: 20px;
  left: 20px;
  z-index: 1;
  }
  .contact-window-stack {
  position: absolute;
  top: 80%;
  left: 70%;
  transform: translateY(-50%);
  width: 300px;
  height: 300px;
  z-index: 15;
  transition: all 0.3s ease;
  }
  .contact-window {
  width: 100%;
  height: 100%;
  position: absolute;
  border: 2px solid #000000;
  box-shadow: 2px 2px 0 rgba(0, 0, 0, 0.5);
  background: white;
  }
  .contact-window:nth-child(1) {
  top: 0;
  left: 0;
  z-index: 3;
  }
  .contact-window:nth-child(2) {
  top: 10px;
  left: 10px;
  z-index: 2;
  }
  .contact-window:nth-child(3) {
  top: 20px;
  left: 20px;
  z-index: 1;
  }
  .search-window-stack {
  position: absolute;
  top: 10px;
  left: 10px;
  width: 200px;
  height: 200px;
  z-index: 1000;
  display: block !important;
  padding: 2%
  }
  #job-popup .content {
  overflow: hidden;
  margin-left: 10px;
  top: 80px;
  }
  #job-popup .content {
  margin-top: 10px;
  padding: auto;
  }
  #search-window .content {
  top: 80px;
  }
  .search-window {
  width: 100%;
  height: 100%;
  position: absolute;
  border: 2px solid #000000;
  box-shadow: 2px 2px 0 rgba(0, 0, 0, 0.5);
  background: white;
  }
  .search-window:nth-child(1) {
  top: 0;
  left: 0;
  z-index: 3;
  }
  .search-window:nth-child(2) {
  top: 7px;
  left: 7px;
  z-index: 1;
  }
  .search-window:nth-child(3) {
  top: 18px;
  left: 20px;
  z-index: 1;
  }
  .about-window:hover,
  .projects-window:hover,
  .cv-window:hover,
  .contact-window:hover,
  .search-window:hover {
  z-index: 100;
  transform: scale(1.02);
  box-shadow: 4px 4px 0 rgba(0, 0, 0, 0.3);
  }

  .projects-grid {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(280px, 1fr));
  gap: 25px;
  padding: 20px;
  }

  .project-card {
  border: 2px solid #000;
  overflow: hidden;
  transition: all 0.3s ease;
  background: #fff;
  position: relative;
  }
  .project-card:hover {
  transform: translateY(-5px);
  box-shadow: 0 5px 15px rgba(0, 0, 0, 0.3);
  }
  .project-image {
  position: relative;
  overflow: hidden;
  }
  .project-card img {
  width: 100%;
  height: 200px;
  object-fit: cover;
  transition: transform 0.3s ease;
  }
  .project-card:hover img {
  transform: scale(1.05);
  }
  .project-info {
  padding: 20px;
  }

  .project-title {
  font-weight: bold;
  margin-bottom: 10px;
  font-size: 20px;
  }
 
  .project-detail {
  position: fixed;
  top: 0;
  left: 0;
  width: 100%;
  height: 100%;
  background: #cccdd1;
  z-index: 1000;
  display: none;
  overflow-y: auto;
  }
  .project-detail.active {
  display: block;
  }
  .back-button {
  position: fixed;
  top: 20px;
  left: 20px;
  display: flex;
  align-items: center;
  gap: 5px;
  background: none;
  border: none;
  cursor: pointer;
  font-family: "PP Mondwest";
  font-size: 22px;
  z-index: 1001;
  }
  .back-button {
    color: #000;
  }
  .project-detail-content {
  max-width: 800px;
  margin: 60px auto 20px;
  padding: 20px;
  }
  
  .project-detail-title {
  font-size: 26px;
  margin-bottom: 20px;
  }

  .project-detail-image {
  width: 100%;
  max-height: 500px;
  object-fit: cover;
  margin-bottom: 70px;
  }

  .project-detail-video {
  width: 100%;
  margin-bottom: 20px;
  }

  .project-detail-video-container {
  position: relative;
  padding-bottom: 56.25%;
  height: 0;
  overflow: hidden;
  margin-bottom: 20px;
  }
  .project-detail-video-container iframe {
  position: absolute;
  top: 0;
  left: 0;
  width: 100%;
  height: 100%;
  margin-bottom: 20px;
  }
  .section-window .window-content {
  overflow-y: auto;
  max-height: calc(80vh - 40px);
  }
  .photo-grid {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(300px, 1fr));
  gap: 20px;
  margin-bottom: 20px;
  }
  .photo-item {
  border-radius: 8px;
  overflow: hidden;
  box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1);
  }
  .photo-item img {
  width: 100%;
  height: auto;
  display: block;
  transition: transform 0.3s ease;
  }
  .photo-item:hover img {
  transform: scale(1.05);
  }
  /* Contact Links */
  .contact-links {
  display: flex;
  flex-direction: column;
  gap: 15px;
  }
  .contact-link {
  display: flex;
  align-items: center;
  gap: 10px;
  padding: 10px;
  border: 2px solid #000;
  border-radius: 5px;
  text-decoration: none;
  color: #000;
  transition: all 0.3s ease;
  background: #fff;
  }
  .contact-link:hover {
  background-color: rgba(0, 0, 0, 0.1);
  transform: translateY(-2px);
  box-shadow: 0 1px 10px rgba(0, 0, 0, 0.1);
  }
  /* CV Content */
  .cv-content {
  padding: 20px;
  }
  .cv-content h3 {
  font-size: 20px;
  margin-bottom: 15px;
  margin-top: 25px;
  }
  
  .cv-content h3:first-child {
  margin-top: 0;
  }

  .cv-content ul {
  list-style-type: none;
  margin-left: 20px;
  }

  .cv-content li {
  margin-bottom: 10px;
  position: relative;
  }
  .cv-content li:before {
  content: "•";
  position: absolute;
  left: -15px;
  }

  .download-cv-btn {
  display: flex;
  align-items: center;
  gap: 10px;
  text-decoration: none;
  color: #000000;
  padding: 10px;
  border: 2px solid #000;
  border-radius: 5px;
  margin-top: 20px;
  transition: all 0.3s ease;
  background: #fff;
  width: fit-content;
  }

  .download-cv-btn:hover {
  background-color: rgba(0, 0, 0, 0.1);
  transform: translateY(-2px);
  }
  
  @keyframes fadeIn {
  from { opacity: 0; }
  to { opacity: 1; }
  }

  
  .close-btn {
  position: absolute;
  top: 10px;
  right: 25px;
  font-size: 20px;
  cursor: pointer;
  color: white !important;
  -webkit-text-stroke: 0 !important;
  z-index: 10;
  }

  .project-detail-video {
  width: 100%;
  margin: 20px 0;
  display: flex;
  justify-content: center;
  align-items: center;
  }
  .project-detail-video-container {
  width: 100%;
  max-width: 800px;
  background: #000;
  overflow: hidden;
  }
  .project-detail-video-container video {
  width: 100%;
  height: auto;
  display: block;
  image-rendering: crisp-edges;
  -webkit-transform: translateZ(0);
  transform: translateZ(0);
  -webkit-media-controls {
  display: none !important;
  }

  }
  .project-detail-video-container video::-webkit-media-controls-enclosure {
  display: none !important;
  }

  .project-detail-video-container video::-webkit-media-controls-panel {
  display: none !important;
  }
  .project-detail-video-container video::-webkit-media-controls-play-button,
  .project-detail-video-container video::-webkit-media-controls-mute-button,
  .project-detail-video-container video::-webkit-media-controls-timeline,
  .project-detail-video-container video::-webkit-media-controls-current-time-display,
  .project-detail-video-container video::-webkit-media-controls-time-remaining-display,
  .project-detail-video-container video::-webkit-media-controls-fullscreen-button {
  display: none !important;
  }


  
  .project-detail {
  position: fixed;
  top: 0;
  left: 0;
  width: 100%;
  height: 100%;
  background: white;
  padding: 20px;
  z-index: 1000;
  overflow-y: auto;
  animation: slideInRight 0.4s cubic-bezier(0.4, 0, 0.2, 1);
  }
  @keyframes slideInRight {
  from {
  transform: translateX(100%);
  }
  to {
  transform: translateX(0);
  }
  }
  .project-detail-video {
  width: 100%;
  max-width: 800px;
  margin: 20px auto;
  border-radius: 12px;
  overflow: hidden;
  box-shadow: 0 12px 40px rgba(0, 0, 0, 0.15);
  }

 
  .interactive-project-container {
  display: flex;
  flex-direction: column;
  border: 1px solid #e0e0e0;
  border-radius: 8px;
  overflow: hidden;
  margin-bottom: 24px;
  background-color: #fff;
  box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1);
  }
  .code-tabs {
  display: flex;
  background-color: #f5f5f5;
  border-bottom: 1px solid #e0e0e0;
  }
  .tab-button {
  padding: 12px 20px;
  border: none;
  background: none;
  cursor: pointer;
  font-size: 14px;
  font-weight: 500;
  color: #555;
  transition: all 0.2s ease;
  }
  .tab-button:hover {
  background-color: rgba(0, 0, 0, 0.05);
  }
  .tab-button.active {
  color: #a1a9b2;
  border-bottom: 2px solid #007bff;
  background-color: #fff;
  }
  .tab-content {
  display: none;
  padding: 16px;
  overflow: auto;
  max-height: 500px;
  }
  .tab-content.active {
  display: block;
  }
  .tab-content pre {
  margin: 0;
  padding: 16px;
  background-color: #f8f9fa;
  border-radius: 4px;
  overflow-x: auto;
  }



  .project-detail-video-container video {
    width: 100%;
    height: 100%;
    object-fit: cover; /* Remplit en rognant */
  }
  


/* Supprime tous les contrôles natifs */
.project-detail-video-container video::-webkit-media-controls-enclosure {
  display: none !important;
}

.project-detail-video-container video::-webkit-media-controls-panel {
  display: none !important;
}

.project-detail-video-container video::-webkit-media-controls-play-button,
.project-detail-video-container video::-webkit-media-controls-mute-button,
.project-detail-video-container video::-webkit-media-controls-timeline,
.project-detail-video-container video::-webkit-media-controls-current-time-display,
.project-detail-video-container video::-webkit-media-controls-time-remaining-display,
.project-detail-video-container video::-webkit-media-controls-fullscreen-button {
  display: none !important;
}

@media (max-width: 768px) {
  .project-detail-video-container {
      border-radius: 0;
  }
  .project-detail-video {
      margin: 10px -15px;
  }
}

.photo-gallery-container {
  width: 100%;
  max-width: 1200px;
  margin: 0 auto;
  position: relative;
}



@keyframes slideInRight {
  from {
    transform: translateX(100%);
  }
  to {
    transform: translateX(0);
  }
}

.project-detail-video {
  width: 100%;
  max-width: 800px;
  margin: 20px auto;
  overflow: hidden;
  box-shadow: 0 12px 40px rgba(0, 0, 0, 0.15);
}

.photo-gallery {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(220px, 1fr));
  gap: 16px;
  margin: 24px 0;
}

.photo-gallery .photo-item {
  display: block;
  border-radius: 12px;
  transition: all 0.3s ease;
}

.photo-gallery .photo-item:hover {
  transform: translateY(-4px);
  box-shadow: 0 12px 35px rgba(0, 0, 0, 0.15);
}

.description {
  white-space: pre-line;
  margin-top: 24px;
  line-height: 1.6;
  color: #333;
  font-size: 16px;
}


@keyframes spin {
  0% { transform: rotate(0deg); }
  100% { transform: rotate(360deg); }
}


@media (max-width: 900px) {
  body {
      font-size: 15px;
      padding: 0;
      overflow-x: hidden;
      display: flex;
      justify-content: center;
      align-items: center;
    }


  h1 {
    font-size: 23px;
  }
  
  
  .desktop {
      width: 200px;
      height: 100px;
      padding: 20px;
      position: relative;
      display: flex;
      align-items: center;
  }

  .search-window {
    height: 120px;
    width: 210px;
  }
  

  /* CENTRE LE PORTFOLIO */
  .project-window-stack {
    position: absolute; 
    left: 15%;
    top: 40%;
    transform: translate(-50%, -50%);   
    width: 90px;
    height: 250px;
    padding: 10%;
  }

  .projects-window {
      font-size: 20px;
      width: 330px;
      height: 700px;
      min-height: 550px;
  }

   
  .folder-grid {
      width: auto;
      grid-template-columns: repeat(2, 1fr);
      gap: 4px;
      padding: 10px;
      height: 200px;
  }


  .folder {
      padding: 5px;
  }

  .folder-icon {
      width: 50px;
      height: 40px;
  }

  .folder span {
      font-size: 8px;
  }

  /* PETITES FENETRES STACK SANS ROTATION */
 

  .about-window-stack {
      top: 10px;
      left: 110px;
      width: 250px;
     
  }
  

  .cv-window-stack {
      top: 65%;
      left: 10px;
      width: 230px;
      height: 110px;

  }

  .contact-window-stack {
      position: absolute;
      top: 80%;
      left: 30%;
      width: 220px;
      height: 100px;
  }
  
  /* ✅ Correctif spécifique Safari */
  @supports (-webkit-touch-callout: none) {
      .contact-window-stack {
          top: 72%; /* ajuste uniquement dans Safari */
      }
  }
  
  /* Style stack simple */
  .about-window, 
  .cv-window,
  .contact-window {
      width: auto nt;
      height: auto !important;
      transform: none !important;
      border: 1px solid #000;
  }

  .about-window:nth-child(1),
  .cv-window:nth-child(1),
  .contact-window:nth-child(1) {
      top: 0;
      left: 0;
  }

  .about-window:nth-child(2),
  .cv-window:nth-child(2),
  .contact-window:nth-child(2) {
      top: 8px;
      left: 8px;
  }

  .about-window:nth-child(3),
  .cv-window:nth-child(3),
  .contact-window:nth-child(3) {
      top: 14px;
      left: 16px;
  }

  /* Fenêtres modales */
  .window {
      font-size: 18px;
      width: 360px !important;
      height: 250px !important;
      top: auto !important;
      left: 50% !important;
  }

  .window-header {
      padding: 8px 12px;
  }

  .window-title {
      font-size: 23px;
  }

  .window-content {
      padding: 10px;
      max-height: 60vh;
      overflow-y: auto;
      font-size: 19px;
  }
}



/* Interactive Projects Styles */
.interactive-project-container {
  height: 300px;
  border: 1px solid #e0e0e0;
  border-radius: 8px;
  margin-bottom: 24px;
  box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1);
}

.code-tabs {
  display: flex;
  background-color: #f5f5f5;
  border-bottom: 1px solid #e0e0e0;
}

.tab-button {
  padding: 12px 20px;
  border: none;
  background: none;
  cursor: pointer;
  font-size: 14px;
  font-weight: 500;
  color: #555;
  transition: all 0.2s ease;
}

.tab-button:hover {
  background-color: rgba(0, 0, 0, 0.05);
}

.tab-button.active {
  color: #007bff;
  border-bottom: 2px solid #007bff;
  background-color: #fff;
}

.tab-content {
  display: none;
  padding: 16px;
  overflow: auto;
  max-height: 500px;
}

.tab-content.active {
  display: block;
}

.tab-content pre {
  margin: 0;
  padding: 16px;
  background-color: #f8f9fa;
  border-radius: 4px;
  overflow-x: auto;
}

.tab-content code {
  font-family: 'Courier New', Courier, monospace;
  font-size: 14px;
  line-height: 1.5;
  color: #333;
}

.preview-container {
  width: 100%;
  min-height: 500px;
  display: flex;
  justify-content: center;
  align-items: center;
  overflow: hidden;
}

/* For code highlighting */
.language-html {
  color: #e34c26;
}

.language-css {
  color: #563d7c;
}

.language-javascript {
  color: #f1e05a;
}



.player-window {
  width: 600px; /* fixe largeur desktop */
  max-width: 100%; /* permet réduction sous 600px */
}

@media (max-width: 768px) {
  .container {
      max-width: 95vw; /* presque toute la largeur écran */
      padding: 20px 10px;
      position: absolute;
      top: 50%;
      left: 50%;
      transform: translate(-50%, -50%);
  }
  .player-window {
      width: 100%; /* prendre toute la largeur de container */
      max-width: none; /* enlever max width */
      border-radius: 6px;
  }
}


/* Media Queries pour le responsive */
@media (max-width: 900px) {
  #content-wrapper {
    flex-direction: column;
    align-items: center;
  }
  
  #wrap {
    margin-right: 0;
  }
  
  .simple-gallery {
    margin-left: 0;
    padding-right: 0;
  }
}

@media (max-width: 600px) {
  #wrap {
    width: 200px;
    height: 200px;
  }
  
  #box > div {
    width: 50px;
    height: 50px;
  }
  
  div#cd {
    transform: translateZ(-4px) translateX(70px);
    border-radius: 200px;
    box-shadow: 0 0 0 3px silver inset,
                0 0 0 60px #f6f6f6 inset,
                0 0 0 63px silver inset,
                0 0 0 85px rgba(255,255,255,0.25) inset;
  }
  
  div#right { left: 192px; }
  div#bottom { top: 192px; }
  
  .gallery-grid {
    grid-template-columns: repeat(auto-fill, minmax(120px, 1fr));
  }
  
  .gallery-grid img {
    height: 120px;
  }
}

@media (max-width: 400px) {
  #wrap {
    width: 150px;
    height: 150px;
  }
  
  #box > div {
    width: 150px;
    height: 150px;
  }
  
  div#cd {
    transform: translateZ(-4px) translateX(50px);
    border-radius: 150px;
    box-shadow: 0 0 0 3px silver inset,
                0 0 0 45px #f6f6f6 inset,
                0 0 0 48px silver inset,
                0 0 0 65px rgba(255,255,255,0.25) inset;
  }
  
  div#right { left: 142px; }
  div#bottom { top: 142px; }
}

/* OVERRIDE FORCE - À ajouter à la fin de ton fichier CSS */
.interactive-project-layout {
  display: grid !important;
  gap: 65px !important; /* Espacement personnalisable */
  align-items: start !important;
}

.cd-container {
  width: 90% !important;
  max-width: 300px !important;
  margin: 0 !important;
  justify-self: center !important;
}

.project-gallery {
  width: 100% !important;
  max-width: 550px !important;
  margin: 0 !important;
  padding: 0 !important;
  overflow: hidden !important; /* Contrôle le scroll interne */

}

/* Reset des médias queries problématiques */
@media (max-width: 992px) {
  .interactive-project-layout {
    grid-template-columns: 1fr !important;
    gap: 40px !important;
  }
  
  .cd-container, 
  .project-gallery {
    max-width: 100% !important;
  }
}


.project-detail .project-description-container {
  margin-top: 10px !important;    
  padding-top: 10px !important;     
}
